...
| **Deploy workflow (`deploy-app.yml`)** | Sole trigger: **`workflow_run`** after workflow **`CI`** completes with **`conclusion == success`** on **`main`**. **Security (Task Lock [21]):** gate only when triggering CI **`event == push`** and **`head_repository.full_name == github.repository`** (no PR/fork privileged deploy). Checkout **`github.event.workflow_run.head_sha`** with **`fetch-depth: 1`**, **`persist-credentials: false`**. Validate **`IMAGE_TAG`** as **7-char lowercase hex** before docker/Trivy. First job: **paths-filter** — skip unless **`app/**`**, **`app/Dockerfile`**, **`compose.yml`**, or **`.github/workflows/deploy-app.yml`** changed. **`environment: production`**. OIDC via **`azure/login`** on build job only; job permissions **`id-token: write`** + **`contents: read`** (gate job **`contents: read`** only). Trivy **before** ACR push. |
...
| **Container image scan (Trivy)** | Scan **built image** in deploy workflow; **fail on CRITICAL** with a **published fix** (`ignore-unfixed: true` skips base-OS CVEs awaiting upstream patches). |
...
| **Container build layout** | **`app/Dockerfile`** + **`app/.dockerignore`** (build context = `app/`). Root **`compose.yml`** with `build.context: ./app`. CI `docker build` uses `app/` as context. Monorepo root is not the image build context. |
| **Container image build** | **Multi-stage** on **`python:3.12-slim`**: **builder** = **`ghcr.io/astral-sh/uv:python3.12-bookworm-slim`** (pinned tag, not `:latest`); **cache-friendly COPY** — `pyproject.toml` + `uv.lock` → **`uv sync --frozen --no-dev --no-install-project`**; then **`src/`** → **`uv sync --frozen --no-dev`**. **Runtime** = **`python:3.12-slim`**; copies **`.venv` + `src/`** only. **`app/.dockerignore`** excludes **`tests/`**, **`.venv/`**, caches (**`.pytest_cache/`**, **`.mypy_cache/`**, **`.ruff_cache/`**), **`__pycache__/`**, **`.git/`**. |
| **Container runtime user** | Dedicated non-root user **`appuser`** with UID/GID **10001**. **`WORKDIR /app`**. Process starts via **`.venv/bin/uvicorn`** (not `uv run`). **`EXPOSE 8000`** (documentation; bind **`0.0.0.0:8000`**). **Exec-form CMD:** `weather_api.main:create_app --factory --host 0.0.0.0 --port 8000`. |
| **Docker Compose (local)** | Root **`compose.yml`**, service **`weather-api`**, **`image: weather-api:local`**, **`ports: ["8000:8000"]`**, **`restart: unless-stopped`**. **Inline `environment` only** (no `env_file`) — **`WEATHER_PROVIDER=mock`**, **`ENABLE_HSTS=false`**; no secrets required for default `docker compose up`. **Compose healthcheck only** (not Dockerfile): stdlib Python GET **`/health/live`** on **`127.0.0.1:8000`**. ACA probes defined in Terraform (Task 17). |
| **Application listen config** | Default port **8000**; **`PORT`** env parsed into settings but **container/ACA bind port is fixed 8000** via uvicorn CLI (Phase Lock [2]). Bind **`0.0.0.0`**. Uvicorn serves **`weather_api.main:create_app`** with **`--factory`**. FastAPI **`debug=False`** in production. |
...
- **Container build layout (2025-06-02, Phase Lock [2]):** **`app/Dockerfile`** + **`app/.dockerignore`**; root **`compose.yml`** with `build.context: ./app` — Option A.
- **Container image build (2025-06-02, Phase Lock [2]):** **Multi-stage `python:3.12-slim`**; **`uv` bookworm builder**; cache-friendly COPY; runtime **`.venv` + `src`** only.
- **Container runtime user (2025-06-02, Phase Lock [2]):** **`appuser`** UID/GID **10001**; **`WORKDIR /app`**; **`.venv/bin/uvicorn`** entry; **`EXPOSE 8000`**.
- **Container start command (2025-06-02, Phase Lock [2]):** **Exec-form CMD** — `create_app --factory --host 0.0.0.0 --port 8000`; fixed port (not `${PORT}` shell wrapper).
- **Docker Compose local (2025-06-02, Phase Lock [2]):** Service **`weather-api`**, **`weather-api:local`**, inline env only (**mock**, no `env_file`); port **8000:8000**.
- **Container healthcheck (2025-06-02, Phase Lock [2]):** **Compose-only** healthcheck via Python stdlib → **`/health/live`**; no Dockerfile **`HEALTHCHECK`**; ACA probes in Terraform.
- **Docker build context (2025-06-02, Phase Lock [2]):** **`app/.dockerignore`** production-focused — exclude **tests**, **`.venv`**, caches, **`__pycache__`**, **`.git`**.
